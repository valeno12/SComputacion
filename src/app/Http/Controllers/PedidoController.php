...
class PedidoController extends Controller
{
    public function index(Request $request)
    {
        ...
        $pedidos = Pedido::whereIn('estadoActual_id', [1, 2, 3])
    ...
    public function finalizados(Request $request)
    {
        ...
        $pedidos = Pedido::where('estadoActual_id', 4)
                    ->where(function ($queryBuilder) use ($query) {
                        $queryBuilder->where('codigo', 'ilike', '%' . $query . '%')
                            ->orWhereHas('cliente', function ($clienteQuery) use ($query) {
                                $clienteQuery->where('nombre', 'ilike', '%' . $query . '%')
        ...
        return view('pedido.finalizados', compact('pedidos'));
    }
    ...
    public function entregados(Request $request)
    {
        ...
        $pedidos = Pedido::where('estadoActual_id', 5)
                    ->where(function ($queryBuilder) use ($query) {
                        $queryBuilder->where('codigo', 'ilike', '%' . $query . '%')
                            ->orWhereHas('cliente', function ($clienteQuery) use ($query) {
                                $clienteQuery->where('nombre', 'ilike', '%' . $query . '%')
        ...
        return view('pedido.entregados', compact('pedidos'));
